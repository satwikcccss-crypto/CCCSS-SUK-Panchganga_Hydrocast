...
def sync_to_supabase(state: dict, db_url: str):
    ...
    using the unified synchronizer.
    """
    try:
        from src.db.connection import get_db_connection
        from src.db.sync_all_to_supabase import apply_schema, sync_single_run
        ...
        apply_schema(conn)
        sync_single_run(conn, state)
        conn.close()
        cycle_id = state.get("status", {}).get("last_cycle", {}).get("run_id", "active")
        log.info("✓ Telemetry successfully synced to Supabase Postgres (run_id: %s)", cycle_id)
    except Exception as e:
        log.error("Supabase sync failed: %s", e, exc_info=True)
